...
const Appointment = () => {
  ...
  const onSubmit = async (values, actions) => {
    ...
    console.log(actions);
    console.log("submit kısmına girdi...");
    ...
    <div className=' min-h-screen w-full flex justify-center items-center' style={{ backgroundImage: "url('./background.jpeg')", backgroundSize: "cover" }}>
      <div className=' w-[90%] md:w-[50%] bg-transparent backdrop-blur-3xl p-5 rounded-lg border'>
        ...
        <form className='flex flex-col poppins-light' onSubmit={handleSubmit}>
          ...
          <label className='text-white' htmlFor="nameSurname">İsim-Soyisim</label>
          <input
            ...
            {errors.nameSurname && <span className='text-red-500 text-sm pl-2 mt-1 font-[400]'>{errors.nameSurname}</span> }
          <label className='text-white mt-2' htmlFor="phone">Telefon Numarası</label>
          <input
            value={values.phone}
            onChange={handleChange}
            onBlur={handleBlur}
            ...
            {errors.phone && <span className='text-red-500 text-sm pl-2 mt-1 font-[400]'>{errors.phone}</span> }
          <label className='text-white mt-2' htmlFor="email">E-posta</label>
          <input
            value={values.email}
            onChange={handleChange}
            onBlur={handleBlur}
            ...
            {errors.email && <span className='text-red-500 text-sm pl-2 mt-1 font-[400]'>{errors.email}</span> }
          <label className='text-white mt-2' htmlFor="clock">Aranmak İstediğiniz Saat</label>
          <input
            value={values.clock}
            onChange={handleChange}
            onBlur={handleBlur}
            ...
            {errors.clock && <span className='text-red-500 text-sm pl-2 mt-1 font-[400]'>{errors.clock}</span> }
          <label className='text-white mt-2' htmlFor="note">Eklemek İstediğiniz Not</label>
          <textarea
            value={values.note}
            onChange={handleChange}
            onBlur={handleBlur}
          ...
          <button disabled={isSubmitting} type='submit' className={`bg-[#E19930] mt-4 hover:bg-yellow-600 duration-150 rounded-md py-2 text-white ${isSubmitting ? 'opacity-50':'opacity-100'}`}>Gönder</button>
        </form>
